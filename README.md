...
1.19.36 finishes the readability pass over every shared clock and calendar name without changing the underlying chronology. Specialist shorthand is expanded where it could mislead a normal user: Roman seasonal time now says that its daylight hours are unequal and that the four night units are military watches; the ancient Babylonian mode states its sunset-start day and fixed bēru/UŠ hierarchy; Renaissance European “Babylonian” hours are explicitly distinguished from that ancient system; the historical Nuremberg clock explains its fixed-date day/night changes; and the location-based reconstruction says that it is not the civic schedule. Calendar names now expose important epoch/model assumptions directly, including the Nabonassar Egyptian era, the traditional Armenian wandering-year era, the full Goodman–Martínez–Thompson Mayan correlation name, and the deterministic Romme continuation used after the historically observed French Republican years. Local sidereal time is labelled as the compact J2000 approximation that Common actually computes rather than being presented as an ephemeris-grade result.

## Common 1.19.35

1.19.35 completes the readability pass over the shared time/calendar catalogue. Chinese double-hour elapsed values now use native 日/時辰 hierarchy; Edo unequal-hour elapsed values normalize complete twelve-toki cycles into 日; and ancient Babylonian elapsed values normalize twelve bēru into an ūmu before residual bēru/UŠ. Selector labels now expose the 23:00 Zǐ convention, dynasty-varying finer hundred-kè subdivisions, native Roman hora/vigilia terminology, and the day/night nature of the historical Nuremberg clock. Calendar labels now make several era/epoch assumptions visible instead of leaving culturally broad names unexplained.

## Common 1.19.34

1.19.34 completes the historical-model pass rather than treating a broad cultural label as one timeless clock. Edo Japanese unequal time is now explicitly split between an early sunrise/sunset model and Takahashi Yoshitoki's documented 1797 twilight definition (Sun centre 7°21′40″ below the horizon). Apparent solar time is labelled as the compact astronomical approximation that the implementation actually computes. Chinese hundred-kè, ancient Babylonian bēru/UŠ and Indian ghaṭī labels now expose their unit assumptions directly. Calendar labels also identify important model/range limits such as modern Japanese eras, the Umm al-Qura table range, computational Chinese/Dangi calendars and modern Badíʿ Naw-Rúz determination.

## Common 1.19.33

1.19.33 completes a presentation audit of the time/calendar catalogue. Technical and historical choices now say what they actually compute: Roman seasonal hours and watches, Edo unequal hours, Renaissance European “Babylonian” hours versus ancient bēru/UŠ, model-dependent Mayan GMT correlation, historical French continuation, Byzantine year boundary, and other non-obvious calendar conventions. Indian ghaṭī output now uses readable native hierarchy (`ghaṭī`, `pala`, and complete `dina`) instead of the opaque `GH hh:mm` hybrid.

## Common 1.19.32

1.19.32 removes the redundant `italian-modern` clock mode. Modern Italian civil time is ordinary standard civil time and is already represented by the existing standard 24-hour clock together with the user's Italy time-zone/locale settings. The separate `italian-hours` mode remains only for the genuinely different historical sunset-origin system.
