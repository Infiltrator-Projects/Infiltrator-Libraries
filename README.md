...
The current release contains **Infiltratr Common 1.6.0**, the reusable C11
...
`src/core.c`, `src/arithmetic.c` and `src/format.c` are independent of GLib,
GTK and operating-system APIs. The `portable` build contains those three
modules only. `src/posix.c` is the Linux/POSIX provider.
...
`build/libinfiltratr-common.so.1.6.0` with SONAME
