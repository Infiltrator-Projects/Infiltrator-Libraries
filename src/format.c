...
char *infiltratr_format_disk_capacity(uint64_t bytes, char *buffer, size_t size)
{
    ...
    size_t unit = 0U;
    uint64_t divisor = 1U;
    while (bytes / divisor >= 1024U && unit + 1U < ARRAY_LENGTH(units)) {
        divisor *= 1024U;
        unit++;
    }
                                ...
                                bool per_second, char *buffer, size_t size)
{
    ...
    if (!isfinite(value) || value < 1024.0L) {
        ...
        value /= 1024.0L;
        while (value >= 1024.0L && unit + 1U < ARRAY_LENGTH(byte_units)) {
            value /= 1024.0L;
            unit++;
        }
    }
                                     ...
                                     char *buffer, size_t size)
{
    ...
    if (!isfinite(maximum) || maximum < 1024.0L) {
        ...
        receive /= 1024.0L;
        maximum /= 1024.0L;
        while (maximum >= 1024.0L && unit + 1U < ARRAY_LENGTH(byte_units)) {
            send /= 1024.0L;
            receive /= 1024.0L;
            maximum /= 1024.0L;
            unit++;
                                        ...
                                        char *buffer, size_t size)
{
    ...
    size_t unit = 0U;
    while (value >= 1024.0L && unit + 1U < ARRAY_LENGTH(units)) {
        value /= 1024.0L;
        unit++;
    }
